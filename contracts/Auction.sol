...
abstract contract AuctionCoreUpgdaeable is // LOW: misspell?
    OwnableUpgradeable,
    IERC721ReceiverUpgradeable
{
    ...
    function setCardAddress(Card _card) external onlyOwner { // NOTE: check what can happen if broken card is passed
        card = _card;
    }

    function setCommission(uint8 commission) external onlyOwner {
        _commission = commission; // MEDIUM: require(commission < 100)
    }
    ...
    function placeCardToAuctionGasFree(
        ...
        bytes32 prefixedHashMessage = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32", // LOW: Could use EIP-712 standard instead
    ...
    function placeBetGasFree(uint256 cardId, uint256 amount, address sender,
        ...
        address signer = ecrecover(prefixedHashMessage, _v, _r, _s);
        require(signer == sender, "snec"); // NOTE: Add check for not equal 0x address?
    ...
    function trimMyBets(address user) public { // NOTE: this function is public, does it mean that anyone can call it and remove items? if so, its critical
...
contract MaintokenAuction is AuctionCoreUpgdaeable {
    ...
    function initialize() external initializer { //NOTE: check the initializing process
        __AuctionCore_init();
    }

    function setMaintokenAddress(MainToken _maintoken) external onlyOwner { // NOTE: Should be added check for the correct maintoken?
...
contract MaticAuction is AuctionCoreUpgdaeable {
    ...
    function _sendPayment(uint256 amount, address receiver) internal override { //NOTE: should receiver be any address?
