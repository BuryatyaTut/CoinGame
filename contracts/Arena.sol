...
contract Arena is IArena, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        __Ownable_init(); // Check how this function works and the initializer
    }

    function setCardAddress(ICard _card) public onlyOwner { // Check if the broken ICard can break the contract
        card = _card;
    }

    function setMainToken(MainToken _maintoken) public onlyOwner { // Check if the broken MainToken can break the contract
        maintoken = _maintoken; // MEDIUM: If the poisoned maintoken will be passed, it can cause problems for the contract. But since it's onlyOwner function it's not so critical
    }
    ...
    function updateEvent(
        uint256 eventId,
        uint256 betsAcceptedUntilTs // NOTE: is it needed a check? Maybe the time should be not less then the current timestamp, cause if we will update time to a lower then the current, what we will do with the bets which was made after the new stop time?
    ) public onlyOwner {
    ...
    function _makeBetCore(
        ...
        bets[cardId] = BetInfo(eventId, choiceId, cardOwner, _betId); // NOTE: is it suppose to be possible for different users rewrite the bets mapping?
    ...
    { // NOTE: maybe add require offset to be less then betsByUser.length | LOW?
    ...
    { // NOTE: same requirement as above| LOW?
    ...
    function claimCall(uint256 callId) public { //NOTE: everyone can call this function
        CallInfo storage thisCall = calls[callId];
        ...
        if (
            ...
            (eventInfos[thisCall.eventId].result == //NOTE: check if choice can be not First/Second won => this will revert => MEDIUM
                invertChoice(thisCall.choice))
        ) {
            ...
            maintoken.mint(winner, reward); // NOTE: check if the reward is 0 => more gas spends => LOW
        } else {
